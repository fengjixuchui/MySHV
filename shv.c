...
INT32 ShvLoad (VOID)
{
    ...
    // This will broadcast a DPC interrupt which will execute the callback routine in parallel on the LPs.
    // Send the callback routine the physical address of the PML4 of the system process, which is what this driver entrypoint should be executing in.
    ...
    return SHV_STATUS_SUCCESS;// Indicate success.
}
