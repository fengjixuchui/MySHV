...
VOID ShvVmxMtrrInitialize (_In_ PSHV_VP_DATA VpData)
{
    UINT32 i;
    ...
    mtrrCapabilities.AsUlonglong = __readmsr(MTRR_MSR_CAPABILITIES);// Read the capabilities mask
    ...
    for (i = 0; i < mtrrCapabilities.VarCnt; i++)
    {
        ...
        {
            VpData->MtrrData[i].PhysicalAddressMin = mtrrBase.PhysBase * MTRR_PAGE_SIZE;// Set the base

            // Compute the length
            _BitScanForward64(&bit, mtrrMask.PhysMask * MTRR_PAGE_SIZE);
            VpData->MtrrData[i].PhysicalAddressMax = VpData->MtrrData[i].PhysicalAddressMin + (1ULL << bit) - 1;
        }
    }
}


UINT32 ShvVmxMtrrAdjustEffectiveMemoryType (_In_ PSHV_VP_DATA VpData, _In_ UINT64 LargePageAddress, _In_ UINT32 CandidateMemoryType)
{
    UINT32 i;
    ...
    for (i = 0; i < sizeof(VpData->MtrrData) / sizeof(VpData->MtrrData[0]); i++)
    {
        ...
        if (VpData->MtrrData[i].Enabled != FALSE)
        {
            // Check if this large page falls within the boundary.
            // If a single physical page (4KB) touches it, we need to override the entire 2MB.
            if (((LargePageAddress + _2MB) >= VpData->MtrrData[i].PhysicalAddressMin) && (LargePageAddress <= VpData->MtrrData[i].PhysicalAddressMax))
            {
                CandidateMemoryType = VpData->MtrrData[i].Type;// Override candidate type with MTRR type
            }
        }
    }

    return CandidateMemoryType;// Return the correct type needed
}
...
VOID ShvVmxEptInitialize (_In_ PSHV_VP_DATA VpData)
{
    ...
    for (i = 0; i < PDPTE_ENTRY_COUNT; i++)
    {
        ...
        for (j = 0; j < PDE_ENTRY_COUNT; j++)
        {
            ...
            VpData->Epde[i][j].Type = ShvVmxMtrrAdjustEffectiveMemoryType(VpData, VpData->Epde[i][j].PageFrameNumber * _2MB, MTRR_TYPE_WB);
        }
    }
}
...
UINT8 ShvVmxEnterRootModeOnVp (_In_ PSHV_VP_DATA VpData)
{
    ...
    return TRUE;// VMX Root Mode is enabled, with an active VMCS.
}
...
VOID ShvVmxSetupVmcsForVp (_In_ PSHV_VP_DATA VpData)
{
    ...
    __vmx_vmwrite(VMCS_LINK_POINTER, ~0ULL);// Begin by setting the link pointer to the required value for 4KB VMCS.
    ...
    if (VpData->EptControls != 0)
    {
        ...
        __vmx_vmwrite(EPT_POINTER, vmxEptp.AsUlonglong);// Load EPT Root Pointer
        __vmx_vmwrite(VIRTUAL_PROCESSOR_ID, 1);// Set VPID to one
    }

    // Load the MSR bitmap. Unlike other bitmaps, not having an MSR bitmap will trap all MSRs, so we allocated an empty one.
    __vmx_vmwrite(MSR_BITMAP, VpData->MsrBitmapPhysicalAddress);

    // Enable support for RDTSCP and XSAVES/XRESTORES in the guest.
    // Windows 10 makes use of both of these instructions if the CPU supports it.
    // By using ShvUtilAdjustMsr, these options will be ignored if this processor does not actully support the instructions to begin with.
    //
    // Also enable EPT support, for additional performance and ability to trap memory access efficiently.
    __vmx_vmwrite(SECONDARY_VM_EXEC_CONTROL, ShvUtilAdjustMsr(VpData->MsrData[11], SECONDARY_EXEC_ENABLE_RDTSCP | SECONDARY_EXEC_XSAVES | VpData->EptControls));

    // Enable no pin-based options ourselves, but there may be some required by the processor. Use ShvUtilAdjustMsr to add those in.
    __vmx_vmwrite(PIN_BASED_VM_EXEC_CONTROL, ShvUtilAdjustMsr(VpData->MsrData[13], 0));
    ...
    __vmx_vmwrite(CPU_BASED_VM_EXEC_CONTROL, ShvUtilAdjustMsr(VpData->MsrData[14], CPU_BASED_ACTIVATE_MSR_BITMAP | CPU_BASED_ACTIVATE_SECONDARY_CONTROLS));

    // If any interrupts were pending upon entering the hypervisor, acknowledge them when we're done.
    // And make sure to enter us in x64 mode at all times
    __vmx_vmwrite(VM_EXIT_CONTROLS, ShvUtilAdjustMsr(VpData->MsrData[15], VM_EXIT_ACK_INTR_ON_EXIT | VM_EXIT_IA32E_MODE));

    // As we exit back into the guest, make sure to exist in x64 mode as well.
    __vmx_vmwrite(VM_ENTRY_CONTROLS, ShvUtilAdjustMsr(VpData->MsrData[16], VM_ENTRY_IA32E_MODE));
    ...
    // because we may be executing in an arbitrary user-mode process right now as part of the DPC interrupt we execute in.
    ...
    // corresponds exactly to the location where RtlCaptureContext will return to inside of ShvVpInitialize.
    ...
    // We give ourselves a standard size kernel stack (24KB) and bias for the context structure that the
    // hypervisor entrypoint will push on the stack, avoiding the need for RSP modifying instructions in the entrypoint.
    // Note that the CONTEXT pointer and thus the stack itself, must be 16-byte aligned for ABI compatibility
    // with AMD64 -- specifically, XMM operations will fail otherwise, such as the ones that RtlCaptureContext will perform.
...
UINT8 ShvVmxProbe (VOID)
{
    ...
    // Check if the Feature Control MSR is locked.
    // If it isn't, this means that BIOS/UEFI firmware screwed up, and we could go around locking it, but we'd rather not mess with it.
    ...
    // The Feature Control MSR is locked-in (valid). Is VMX enabled in normal operation mode?
    if (!(featureControl & IA32_FEATURE_CONTROL_MSR_ENABLE_VMXON_OUTSIDE_SMX))
    {
        return FALSE;
    }

    return TRUE;// Both the hardware and the firmware are allowing us to enter VMX mode.
}


INT32 ShvVmxLaunchOnVp (_In_ PSHV_VP_DATA VpData)
{
    UINT32 i;
